...
# Mean-normalise the residuals (i.e. zero mean)
navdat_resids_clean_scal <- scale(navdat_resids_cleaned[,oxides_list],scale = F)
uncertainty_covariance_matrix <- cov(navdat_resids_clean_scal) 
print(round(uncertainty_covariance_matrix,3))
```

`uncertainty_covariance_matrix` is the multivariate analogy to the standard deviation for our estimated crustal compositions. This is shown in Table S1 in the submitted manuscript.

Now that the distribution of residuals has been defined we can add these to the mean estimates to generate empirical uncertainity distributions. 
...
To generate uncertainties for our smoothed trend we use a bootstrap resampling technique. 
...
Finally we visualise the trend including the bootstrap uncertainties. This smoothed time series is written out as an individual file.
...
### Writing weathering data outfiles 

We write the smoothed weathering series and the $\omega$, $\psi$ data with ages and other metadata to file so that they can be easily accessed.

```{r}
omega_psi_metadata.csv <- data.frame(age_ma = recombined_data$interpreted_age,omega_weathering =recombined_data$omega,psi_provenance = recombined_data$psi,sample_id = recombined_data$sample_id, strat_name = recombined_data$strat_name_long)
write.csv(omega_psi_metadata.csv,"data/omega_psi_metadata.csv",row.names = F)

weathering_time_series_out <-data.frame(age_Ma=smoothed_weathering$x,omega=smoothed_weathering$y,bootstrap_upper_bnd=bootstrap_results$max,bootstrap_lower_bnd=bootstrap_results$min)

write.csv(weathering_time_series_out,"data/weathering_time_series.csv",row.names = F)
```
...
First we calculate the mean ω value for each stratigraphic unit (indicated by the strat_name_long field in the SGP database). To give each unit an age we simply take the mean of all the different ages assigned to that unit in the SGP dataset. Next we smooth this data using a gaussian average as normal.
